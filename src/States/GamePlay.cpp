...
#include "Settings/GamePlayState.hpp"
...
namespace game {
    GamePlay::GamePlay(game_engine::Context* context_) : context(context_) {
        ...
        map = std::make_unique<map::TileMap>(TEXTURE_ID::TILE_SHEET, context, settings::map_size, 
                                             settings::layers_num, settings::grid_size);

        init_player(map->get_spawn_points_coords());
        win_points = map->get_win_points_coords();
    }
...
//-------------------------------------INIT METHODS-------------------------------------------
    ...
    void GamePlay::init_player(std::set<map::TileMap::coord_t> spawn_points) {
        ...
        if (!spawn_points.empty()) {
            ...
            spawn_coords = settings::player_initial_coords;
    ...
    void GamePlay::init_shaders() {
        auto& ass_mgr = context->asset_manager;
        ass_mgr->add_shader(SHADER_ID::BG_MOVING, sf::Shader::Vertex, settings::bg_shader_path);
        
        // TODO use shaders!
        // ass_mgr->add_shader(SHADER_ID::BG_BLACKOUTING, sf::Shader::Fragment, "../assets/shaders/bg_blackput.frag");
        // auto blackout_shader = ass_mgr->get_shader_ptr(SHADER_ID::BG_BLACKOUTING); 
    ...
    void GamePlay::init_textures() {
        ...
        ass_mgr->add_texture(TEXTURE_ID::TILE_SHEET, settings::tile_sheet_texture_path);
        ass_mgr->add_texture(TEXTURE_ID::GAME_FOG_BACK,  settings::game_fog_back_texture_path );
        ass_mgr->add_texture(TEXTURE_ID::GAME_FOG_FRONT, settings::game_fog_front_texture_path);
        ass_mgr->add_texture(TEXTURE_ID::PLAYER_SHEET, settings::player_sheet_texture_path);
        ass_mgr->add_texture(TEXTURE_ID::GAME_BLACKOUT, settings::game_blackout_texture_path);
    ...
    void GamePlay::init_background() {
        ...
        background_back.setTextureRect({0, 0, settings::window_width, settings::window_height});
        background_front.setTextureRect({0, 0, settings::window_width, settings::window_height});

        background_back.setColor(settings::game_bg_back_color);
        background_front.setColor(settings::game_bg_front_color);

        background_blackout.setTextureRect({0, 0, settings::window_width, settings::window_height});
...
//-------------------------------------UPDATE-------------------------------------------
    ...
    void GamePlay::update(const float delta_time) {
        ...
        check_win_and_update();
    }

    void GamePlay::update_shaders(const float delta_time) {
        auto movement_component = player->get_movement_component();

        bg_back_offset  += movement_component->get_velocity() * delta_time / settings::bg_back_offset_coef;
        bg_front_offset += movement_component->get_velocity() * delta_time / settings::bg_front_offset_coef;
    }
        ...
        void GamePlay::update_debug_text() {
                ...
                mouse_coords_text.setPosition({
                    mouse_pos.x + settings::game_debug_mouse_text_indent,
                    mouse_pos.y - settings::game_debug_mouse_text_indent
                });
    ...
    void GamePlay::check_win_and_update() {
        if (exploring_mode)
            return;

        for (auto& coords : win_points) {
            auto tile_rect = sf::FloatRect{
                std::get<0>(coords), std::get<1>(coords),
                map->get_grid_size(), map->get_grid_size()
            };
            
            if (player->get_hitbox_component()->intersects(tile_rect)) {
                exploring_mode = true;
                context->state_manager->add_state(std::make_unique<GameWin>(context));
            }
        }
    }
